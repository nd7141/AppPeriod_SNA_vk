...
methodRun: function(hash, btn, paramsAdd) {
  ...
  var onResponse = function(code) {
    ...
    try {
      ...
      showFastBox({title: getLang('global_error'), width: 500}, '<div style="word-wrap: break-word;">'+code+'</div>').hide();
    }
    return true;
  }
  ajax.post('dev', params, {
  ...
  switch (typeof obj) {
    case 'object':
      ...
      if (Object.prototype.toString.call( obj ) == '[object Array]') {
        ...
        for (var i in obj) {
          items.push('<span class="dev_result_key">'+i+':</span> '+Dev.wrapObject(obj[i], null, i, obj));
        }
    ...
    case 'string':
      ...
      if (obj.match(/^https?:\/\/.*/)) {
        ...
        if(obj.length > 40) {
          displayUrl = obj.substr(0, 17) + '...' + obj.substr(-17);
        } else {
          displayUrl = str;
        }
