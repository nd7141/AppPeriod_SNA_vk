...
function MDropdown(el, options) {
  ...
  MDropdown.prototype.hide = function() {
    ...
    removeEvent(window, 'keydown', MDropdown._window_onKeyPress);
    hide(this.itemsWrap);
    MDropdown._currMDD = null;
    cancelEvent(event);
  }
  ...
  MDropdown.prototype._highlightItem = function(itemId) {
    ...
    for (var i = 0; i < itemsNodes.length; i ++) {
      if (itemsNodes[i].getAttribute('id') == itemId) {
        ...
        this.itemsWrapEl.scrollTop = scrollTop - getSize(this.itemsWrapEl)[1] / 2 + getSize(itemsNodes[i])[1];
        break;
      }
      scrollTop += getSize(itemsNodes[i])[1];
    }
  ...
  MDropdown.prototype._onWheel = function(event) {
    ...
    var itemsWrapElHeight = getSize(this.itemsWrapEl)[1];

    if (itemsContentHeight != itemsWrapElHeight) {
      if (this.itemsWrapEl.scrollTop == 0 && event.deltaY < 0) {
        cancelEvent(event);
      } else if (this.itemsWrapEl.scrollTop == (itemsContentHeight - itemsWrapElHeight) && event.deltaY > 0) {
        cancelEvent(event);
      }
    }
  }
  ...
  MDropdown.prototype._onKeyPress = function(event) {
    ...
    if (event.keyCode == 38 || event.keyCode == 40) {
      ...
      if (hovered) {
        ...
        for (var i = 0; i < this.items.length; i++) {
          if (this.items[i][0] == id) {
            var index = event.keyCode == 38 ? Math.max(i - 1, 0) : Math.min(i + 1, this.items.length - 1);
      ...
      this._doQuickSearch(event);
    }
  }

  MDropdown.prototype._doQuickSearch = function(event) {
    var ch = String.fromCharCode(event.keyCode || event.charCode).toLowerCase();
    var self = this;
  ...
  MDropdown.prototype._initKeypressEvent = function() {
    addEvent(window, 'keydown', MDropdown._window_onKeyPress = this._onKeyPress.bind(this));
  }
  ...
  addEvent(this.itemsWrapEl, 'wheel', this._onWheel.bind(this));

  if (el.hasAttribute('data-value')) {
    var selectedValue = el.getAttribute('data-value');
    this.selectValue(selectedValue, true);
  }
}
