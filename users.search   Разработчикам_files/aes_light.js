...
AdsLight.tryRenderYaDirect = function () {
  ...
  var yaContainer;
  var yaOuterContainer;
  if (!ge(yaContainerId)) {
    debugLog('[YaD] creating container');
    yaOuterContainer = ce('div', {id: yaContainerId + '_outer' }, {display: 'none'});
    yaContainer = ce('div', {id: yaContainerId});
    yaOuterContainer.appendChild(yaContainer);
    var leftAdsContainer = ge('left_ads');
    leftAdsContainer.appendChild(yaOuterContainer);
  }
  yaOuterContainer = ge(yaContainerId + '_outer');
  ...
  Ya.Context.AdvManager.render({
    ...
    onRender: function () { // callback функция, которая вызывается, когда Директ срендерен. То есть в этом колбеке надо наводить красоту с наезжанием блоков друг на друга.
      AdsLight.onYaDirectRenderSuccessful(yaOuterContainer.innerHTML);
    }
...
AdsLight.onYaDirectRenderSuccessful = function (adsHtml) {
  debugLog('[YaD] render successful');
  if (vk__adsLight.yaCloseLink) {
    var closeButton = '<div id="left_hideya_direct" class="left_hide_button" onmouseover="leftBlockOver(this);" onmouseout="leftBlockOut(this);" onclick="leftAdBlockClose(\'ya_direct\', \''+vk__adsLight.yaCloseLink+'\'); return cancelEvent(event);"></div>';
    adsHtml = '<div id="ya_direct" onmouseover="leftBlockOver(\'ya_direct\');" onmouseout="leftBlockOut(\'ya_direct\');">'+closeButton+adsHtml+'</div>';
  }
  AdsLight.showNewBlock(ge('left_ads'), adsHtml, true);
